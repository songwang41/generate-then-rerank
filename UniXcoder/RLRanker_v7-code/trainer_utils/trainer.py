...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        self.current_step = 0
        if self.args.iteration_reranker_steps != 0:
    ...
    def compute_loss_reranker(self, generator_model, reranker_model, inputs, return_outputs=False):
        ...
        # ) # (B*C, max_target_len)
        with torch.no_grad():
            generated_tokens, _ = unwrap_model(generator_model).sample(
                inputs['input_ids'], 
                beam_size = self.args.num_cand_generated,
                max_length = self.args.generator_max_target_length
            )
    ...
    def compute_loss_generator(self, generator_model, reranker_model, inputs, return_outputs=False):
        ...
        generated_tokens, generated_logits = unwrap_model(generator_model).sample(
            inputs['input_ids'], 
            beam_size = self.args.generator_num_cand_generated,
            max_length = self.args.generator_max_target_length
        )
        generated_seqs = self.generator_tokenizer.batch_decode(
                    generated_tokens, skip_special_tokens=True, clean_up_tokenization_spaces=True
        ...
        if self.args.generator_supervised:
            generator_sup_outputs = generator_model(source_ids = inputs['input_ids'], target_ids=inputs['labels'])
    ...
    def prediction_step(
        ...
        if self.args.evaluate_generator or self.args.generate_eval_candidates:
            assert self.args.generate_candidate_strategy in ['beamsearch',  'sampling']
            if self.args.generate_candidate_strategy == 'beamsearch':
                ...
                # group beam search
                generated_tokens,_ = self.generator_model.sample(
                    inputs['input_ids'], 
                    beam_size = self.args.generator_num_cand_generated,
                    max_length = self.args.generator_max_target_length
                )
            ...
            if generated_tokens.shape[-1] < self.args.generator_max_target_length:
