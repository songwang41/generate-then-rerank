...
修改了一下结合结合reranker和metric的reward，在v4的基础上增加了几个参数

```markdown
reranker_reward_scaler = 1.5 #这个reranker给定reward前面乘的系数，如果是0的话就不给reranker的reward
metric_reward_scaler = 1.5 #这个generator给定reward前面乘的系数，如果是0的话就不给reranker的reward
generator_num_cand_generated = 8 #训练generator时采样的样本数量
generator_num_cand_picked = 8 # 训练generator时用来传递reward的样本数量，在现在的方法中，我们要求generator_num_cand_generated = generator_num_cand_picked

\# 但是num_cand_generated和num_cand_picked我们依然设置为16和3
...
python -m torch.distributed.launch --nproc_per_node 8 --master_port 6009 run_train.py --overwrite_output_dir \
    ...
    --reranker_reward_scaler $reranker_reward_scaler --metric_reward_scaler $metric_reward_scaler \
    ...
    --generator_num_cand_generated $generator_num_cand_generated --generator_num_cand_picked $generator_num_cand_picked \
