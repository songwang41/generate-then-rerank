...
def load_generator_model_from_brio(model_args, data_args):
    generator_config = BartConfig.from_pretrained(
        model_args.generator_model_name_or_path,
        revision=model_args.model_revision,
        use_auth_token=True if model_args.use_auth_token else None,
    ...
    generator_model = BartForConditionalGeneration(config=generator_config)

    brio_model = BRIO(model_args.generator_model_name_or_path, generator_tokenizer.pad_token_id, is_pegasus=False)
    
    brio_model.load_state_dict(torch.load(os.path.join(model_args.generator_model_name_or_path, "model_generation.bin"), map_location=torch.device('cpu')))

    generator_model.load_state_dict(brio_model.model.state_dict())
    # generator_model = BartForConditionalGeneration.from_pretrained(
    #     model_args.generator_model_name_or_path,
    #     from_tf=bool(".ckpt" in model_args.generator_model_name_or_path),
    #     config=generator_config,
