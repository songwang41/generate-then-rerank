...
class ReRankingDataset_eval(Dataset):
    ...
    def _extract_sample(self, data):
        ...
        candidates = [(encode_seq(self.tok, x[0]), x[1]) for x in candidates] # only trunk for candidates
    ...
    def __getitem__(self, idx):
        ...
        for c in cands:
            ...
            input_id += c[0][:self.args.max_candidate_length]
...
class ReRankingDataset_train(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cands = [sample['candidates'][0][0]]
        # the neg is always the worst candidate
        cands += [c[0] for c in sample['candidates'][-self.args.max_num+1:]]
