        ...
        if train_flag == 0:  # 0: 训练retriever：
            ...
            retriever_local_ctx_vectors = local_ctx_vectors.reshape(local_q_vector.size(0),
                                                                  local_ctx_vectors.size(0) // local_q_vector.size(
                                                                      0), -1) # (B, C, D)
            retriever_simila = torch.einsum("bh,bdh->bd", local_q_vector, retriever_local_ctx_vectors) #(B, C)
            if args.scale_simmila:
                retriever_dist_p = F.softmax(retriever_simila / (local_q_vector.size(1) ** (1 / 2)), dim=1)
            else:
                retriever_dist_p = F.softmax(retriever_simila, dim=1)  #(B, C)
            with torch.no_grad():
                output_reranker = reranker_model(**inputs_reranker) 
                binary_logits, relevance_logits, _ = output_reranker
                reranker_logits = relevance_logits / args.temperature_normal #(B, C, 1)
                probs = F.softmax(reranker_logits, dim=1)
                reranker_dist_p = probs


                positive_logits = relevance_logits[:,:1] # (B, 1, 1)
                negtive_logits = relevance_logits[:,:] # (B, C, 1)
                positive_logits_expand = positive_logits.expand(negtive_logits.size())
                reward_logits = torch.stack((positive_logits_expand,negtive_logits),-1) # (B, C, 2)
                reward_prob = F.softmax(reward_logits,dim=2)
                reward = torch.log(reward_prob[:,:,0]+eps) # (B, C) 概率reward的实现方法，就是把pos和neg一起算softmax，然后最大化log(p(pos))
            
            # 这里是为了让retriever学习reranker的分布，作为一个知识蒸馏
            ...
            adv_loss = reward * torch.log(retriever_dist_p+eps) #直接用这个reward去乘
