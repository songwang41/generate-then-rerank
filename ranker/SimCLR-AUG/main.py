...
from data_utils import to_cuda, collate_mp, ReRankingDataset, ReRankingDataset_aug
...
def run(rank, args):
    ...
    train_set = ReRankingDataset_aug("./data/%s/train/gen_from_%d"%(args.dataset, args.num_beams), tok, maxlen=args.max_len, maxnum=args.max_num, cache_data = args.cache_data)
...
def main(args):
    # set env
    if args.cuda and len(args.gpuid) > 1:
